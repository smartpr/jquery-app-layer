	...
	if (arg !== undefined) {
		...
		this.each(function() {
			...
			for (key in arg) {
				...
				if (properties[key] instanceof Property) {
			...
			$.each(parent, function(key, property) {
				...
				// setup).
				else if (properties[key] instanceof Property && properties[key].isEmpty()) {
...
$.property.flag = function(component, property, setup) {
	var flagSetup = function(me, key) {
		var component = this;
		if ('element' in component) {
			$(me).bind('change', function(e, to) {
				// console.log(component.element.id, key, "toggleSwitch", to);
				$(component.element).toggleSwitch(key, to);
			});
		}
		// console.log(component.element.id, key, "set value false");
		me.valueOf(false);
	};
	var p;
	
	if (arguments.length > 1) {
		p = $.property.binding(component, property, flagSetup);
	} else {
		setup = component;
		property = undefined;
		component = undefined;
		p = $.property.Boolean(flagSetup);
	}
...
// TODO: What happens if you extend a non-existing property??

$.property.extend = function(setup) {
	return $.component.inherit().setup(setup);
};

$.property.binding = function(component, property, setup) {
	var property = $.component.property(component, property);
...
$.component.property = function() {
	...
	if (arguments.length === 2) {
		...
		var Type = arguments[0] === undefined ? $.al.type.Object : arguments[0];
		property = Property(function() { return new Type(); }, true);
	}
	return property.setup(function(me, key) {
		...
		// TODO: Do the same for `deepchange`?
		$(me).bind('change:before change:fail change:done change', function(e) {
			// If we want the events to bubble up (which we are currently using
			// for container display logic), we need to use trigger here instead
			// of triggerHandler.
			// But; bubbling events can also be annoying (see f.e. #contact-list.display)
			$(component.element).trigger(['component', key, e.type].join(':'), _.toArray(arguments).slice(1));
		});
	});
};
...
$.component.inherit = function() {
	...
	return Property().setup(function(me, key, setupParent) {
...
$.component.flag = function() {
	return $.component.property.apply(this, arguments.length === 0 ? [$.al.wrapper.Boolean] : arguments).setup(function(me, key) {
		var component = this;
		if ('element' in component) {
...
var Component = $.al.subtype({
	...
	construct: function(properties, setup) {
		...
		$.each(properties, function(key, property) {
			...
			self[key] = property instanceof Property ? property : Property(property);
		});
...
// components. ==> In theory perhaps, in reality I doubt if it will ever be
// useful anywhere else.
var Property = $.al.type.Object.subtype({
	...
	construct: function(v, lazy) {
		...
		var _valueOf = this.valueOf,
			isLazy;
		this.valueOf = function(v, lazy) {
			if (arguments.length > 0) {
				isLazy = lazy;
				return _valueOf.call(this, v);
			}
		...
		this.clone = function() {
			var clone = Property(_valueOf.call(this), isLazy);
			for (var i = 0, l = setups.length; i < l; i++) {
