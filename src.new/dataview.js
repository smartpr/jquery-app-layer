	...
	switch (action) {
		...
		case 'set':
			...
			return this.each(function() {
				...
				// console.log('dataview', data.valueOf());
				...
				boundTo = data; //$.al.Conditional(data, opts.condition);
				$template.
					store('dataview', 'set', data).
					store('dataview', 'boundTo', boundTo);
				
				// TODO: DRY up event type construction.
				$([boundTo]).bind('change.dataview' + $template.fetch('dataview', 'id'), function() {
					$template.dataview('set');
				});
				
				var invalidate = [];
				// console.log(data.valueOf() === undefined || _.isArray(data.valueOf()) ? data.valueOf() : [data.valueOf()]);
				$template.before($.flirt($template.fetch('dataview', 'compiled'), data.valueOf(), function(item) {
					var $nodes = this;
					
					boundTo = item; //$.al.Conditional(item, opts.condition);
					...
					if (boundTo instanceof Object) {	// boundTo may be a string (in case of list of emails f.e.)
						$([boundTo]).bind('change.dataview' + $template.fetch('dataview', 'id'), function() {
							$nodes.
		...
		case 'clear':
			...
			return this.each(function() {
				...
				if ($template.fetch('dataview', 'set') !== undefined) {
					$([$template.fetch('dataview', 'boundTo')]).unbind('change.dataview' + $template.fetch('dataview', 'id'));
					$template.
						del('dataview', 'set').
					...
					$this.dataview('rendered', opts.template).each(function() {
						...
						$([$(this).fetch('dataview', 'boundTo')]).unbind('change.dataview' + $template.fetch('dataview', 'id'));
					}).remove();
				}
